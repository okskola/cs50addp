...
def evenpalindrome():
    ...
    check50.run("./w1ap4-sumsum abccba").stdout("Yes").exit()

@check50.check(compiles)
def oddpalindrome():
    """check if odd characters palindrome gives output of yes"""
    check50.run("./w1ap4-sumsum abcdcba").stdout("Yes").exit()

@check50.check(compiles)
def shortpalindrome():
    """check if short palindrome gives output of yes"""
    check50.run("./w1ap4-sumsum k").stdout("Yes").exit()

@check50.check(compiles)
def short2palindrome():
    """check if short palindrome gives output of yes"""
    check50.run("./w1ap4-sumsum kk").stdout("Yes").exit()
...
def palindromewithspace():
    ...
    check50.run("./w1ap4-sumsum \"Yawn a more Roman way\"").stdout("Yes").exit()

@check50.check(compiles)
def nopalindrome():
    """check that program outputs No for non-palondrome"""
    check50.run("./w1ap4-sumsum abcdba").stdout("No").exit()

@check50.check(compiles)
def nopalindrome2():
    """check that program outputs No for numeric non-palondrome"""
    check50.run("./w1ap4-sumsum 12").stdout("No").exit()
